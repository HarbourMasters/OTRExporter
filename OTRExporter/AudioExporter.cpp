...
{
	WriteHeader(nullptr, "", writer, static_cast<uint32_t>(SohResourceType::SOH_AudioSample), 2);
...
void OTRExporter_Audio::Save(ZResource* res, const fs::path& outPath, BinaryWriter* writer)
{
	...
	WriteHeader(res, outPath, writer, static_cast<uint32_t>(SohResourceType::SOH_Audio), 2);
	...
	for (size_t i = 0; i < audio->soundFontTable.size(); i++)
	{
		...
		WriteHeader(nullptr, "", &fntWriter, static_cast<uint32_t>(SohResourceType::SOH_AudioSoundFont), 2);
	...
	for (size_t i = 0; i < audio->sequences.size(); i++)
	{
		...
		WriteHeader(nullptr, "", &seqWriter, static_cast<uint32_t>(SohResourceType::SOH_AudioSequence), 2);
